...
###Converting to 3 day prediction tasks
...
from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import TimeDistributed, Conv2D, Flatten, Dense, LSTM, Reshape

def cnn_time_distributed_model():
    model = Sequential([
        TimeDistributed(Conv2D(16, (3, 3), activation='relu', padding='same'), input_shape=(10, 45, 36, 7)),
...
model = cnn_time_distributed_model()
model.summary()
#history=model.fit(X_data_sequences, Y_data_next_3_days, epochs=40, batch_size=10, validation_split=0.2)
model.fit(X_data_sequences, Y_data_next_3_days, epochs40, batch_size=10, validation_split=0.2)


# Save the model in the HDF5 format
model.save("regression_3_B4_Cauveri.h5")  # It will save the model in a single HDF5 file
